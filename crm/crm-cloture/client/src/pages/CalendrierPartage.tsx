...
export function CalendrierPartage() {
  ...
  const updateSchedule = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Calendrier mis à jour", description: "La date d’installation est visible aux ventes et installations." });
    },
  });
  ...
  const monthLabel = month.toLocaleDateString("fr-CA", { month: "long", year: "numeric" });
  ...
  return (
    <>
      <PageHeader
        title="Calendrier partagé ventes & installation"
        description="Vue commune pour voir les dates d’installation, les lieux, les installateurs et les disponibilités avant de promettre une date au client."
        action={
      ...
      <div className="p-6 lg:p-8 space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <KpiCard label="Installations planifiées" value={scheduled.length} icon={<CalendarDays className="h-4 w-4" />} />
          <KpiCard label="À réserver" value={toReserve.length} icon={<Clock className="h-4 w-4" />} accent="warning" />
          <KpiCard label="Capacité/jour" value={baseDailyCapacity} icon={<Users className="h-4 w-4" />} accent="info" />
          <KpiCard label="Disponible sélection" value={selectedAvailable} icon={<Wrench className="h-4 w-4" />} accent={selectedAvailable > 0 ? "success" : "danger"} />
        </div>
        ...
        <div className="grid grid-cols-1 2xl:grid-cols-[1.25fr_0.75fr] gap-6">
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="outline" size="sm" data-testid="button-this-month" onClick={() => setMonth(new Date())}>Aujourd’hui</Button>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <Badge variant={available > 0 ? "outline" : "destructive"} className="text-[9px]">{available} dispo</Badge>
                      </div>
                      <div className="mt-2 space-y-1">
                        ...
                        {jobs.length > 3 && <div className="text-[10px] text-muted-foreground">+{jobs.length - 3} autre(s)</div>}
                      </div>
                    </button>
                  );
                })}
          ...
          <div className="space-y-6">
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-base">Détails du {new Date(`${selectedDate}T12:00:00`).toLocaleDateString("fr-CA", { weekday: "long", day: "numeric", month: "long" })}</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="grid grid-cols-3 gap-2 text-center text-xs">
                  <div className="rounded-md border border-border p-2"><div className="text-muted-foreground">Capacité</div><div className="text-lg font-bold">{baseDailyCapacity}</div></div>
                  <div className="rounded-md border border-border p-2"><div className="text-muted-foreground">Réservé</div><div className="text-lg font-bold">{selectedBooked}</div></div>
                  <div className="rounded-md border border-border p-2"><div className="text-muted-foreground">Disponible</div><div className="text-lg font-bold">{selectedAvailable}</div></div>
                </div>
                {selectedJobs.length === 0 ? (
                  <p className="text-sm text-muted-foreground">Aucune installation planifiée pour cette journée.</p>
                ) : (
                    ...
                    {selectedJobs.map(q => {
                      ...
                      return (
                        <div key={q.id} className="rounded-lg border border-border p-3">
                          ...
                          <div className="mt-1 text-xs text-muted-foreground">Heure : <span className="font-medium text-foreground">{q.scheduledTime || "À confirmer"}</span></div>
                          <div className="mt-1 text-xs text-muted-foreground">Équipe : <span className="font-medium text-foreground">{crew?.name || installer?.name || "Non assignée"}</span></div>
                          {canEditCalendar && (
                            <div className="mt-3 grid grid-cols-1 md:grid-cols-[1fr_1fr_1.4fr] gap-2">
                              ...
                              >
                                <SelectTrigger data-testid={`select-edit-install-crew-${q.id}`}><SelectValue placeholder="Équipe" /></SelectTrigger>
                                <SelectContent>
                                  <SelectItem value="none">Équipe à confirmer</SelectItem>
            ...
            <Card>
              <CardHeader className="pb-3"><CardTitle className="text-base">Équipes disponibles</CardTitle></CardHeader>
              <CardContent>
                <div className="space-y-2">
                  {activeCrews.map(c => (
                    <div key={c.id} className="flex items-center justify-between gap-3 rounded-md border border-border p-2.5">
                      ...
                      <Badge variant="outline" className="text-[10px]">{c.capacity}/jour</Badge>
                    </div>
                  ))}
                  {activeCrews.length === 0 && <p className="text-sm text-muted-foreground">Aucune équipe disponible pour ce filtre.</p>}
                </div>
              </CardContent>
            </Card>
          </div>
        ...
        <Card>
          <CardHeader className="pb-3"><CardTitle className="text-base">Soumissions à réserver dans le calendrier</CardTitle></CardHeader>
          <CardContent>
            {toReserve.length === 0 ? (
              <p className="text-sm text-muted-foreground">Aucune soumission prête à réserver.</p>
            ) : (
                  ...
                  return (
                    <div key={q.id} className="rounded-lg border border-border p-3" data-testid={`calendar-quote-${q.id}`}>
                      ...
                      <div className="mt-2 grid grid-cols-3 gap-2 text-[11px]">
                        <div><span className="text-muted-foreground">Valeur</span><div className="font-bold">{moneyFmt.format(q.estimatedPrice || 0)}</div></div>
                        <div><span className="text-muted-foreground">Capacité zone</span><div className="font-bold">{matchingCapacity}/jour</div></div>
                        <div><span className="text-muted-foreground">Installation</span><div className="font-bold"><StatusBadge status={q.installStatus} /></div></div>
                      </div>
                      <div className="mt-3 grid grid-cols-1 md:grid-cols-[1fr_0.8fr_1.2fr_auto] gap-2">
                        ...
                        <Select
                          ...
                          onValueChange={(v) => updateSchedule.mutate({ id: q.id, scheduledDate: q.scheduledDate || selectedDate, scheduledTime: q.scheduledTime || undefined, installerId: q.assignedInstallerId || undefined, crewId: v === "none" ? null : Number(v) })}
                        >
                          <SelectTrigger data-testid={`select-calendar-crew-${q.id}`}><SelectValue placeholder="Équipe" /></SelectTrigger>
                          <SelectContent>
                            <SelectItem value="none">Équipe à confirmer</SelectItem>
                            {crews.filter(c => crewMatches(c, q, province)).map(c => <SelectItem key={c.id} value={String(c.id)}>{c.name} ({c.province})</SelectItem>)}
                          </SelectContent>
