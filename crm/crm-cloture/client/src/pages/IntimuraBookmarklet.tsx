...
export function IntimuraBookmarklet() {
  ...
  const bookmarkletJs = token
    ? `javascript:(function(){try{var t=document.querySelector('table');if(!t){alert('Aucun tableau trouve sur cette page. Va sur la liste des quotes Intimura.');return;}var heads=[].slice.call(t.querySelectorAll('thead th, thead td')).map(function(h){return (h.textContent||'').trim().toLowerCase();});var trs=t.querySelectorAll('tbody tr');var rows=[];trs.forEach(function(tr){var tds=tr.querySelectorAll('td');if(!tds.length)return;var o={};tds.forEach(function(td,j){var key=heads[j]||('col'+j);o[key]=(td.textContent||'').trim();});var link=tr.querySelector('a[href]');if(link){var href=link.getAttribute('href')||'';o._href=href;var m=href.match(/([0-9a-fA-F-]{8,})/);if(m){o._id=m[1];}else{var n=href.match(/(\\d{2,})/);if(n)o._id=n[1];}}rows.push(o);});if(!rows.length){alert('Le tableau est vide ou les lignes n ont pas pu etre lues.');return;}var payload=JSON.stringify(rows);if(payload.length-1900000===Math.abs(payload.length-1900000)&&payload.length!==1900000){alert('Trop de donnees pour un seul transfert. Filtre la liste sur Intimura.');return;}sessionStorage.setItem('intimura_token','${token}');sessionStorage.setItem('intimura_payload',payload);var url='${apiBase}/#/intimura-receive';window.open(url,'_blank');}catch(e){alert('Erreur bookmarklet: '+(e&&e.message?e.message:e));}})();`
    : "";
